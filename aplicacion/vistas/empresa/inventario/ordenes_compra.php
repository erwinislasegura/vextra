...
<div class="card mt-3">
    ...
    <div class="d-flex flex-wrap gap-2 align-items-center">
      ...
      <form method="GET" class="d-flex gap-2">
        ...
        <select name="estado" class="form-select form-select-sm" style="min-width: 220px;">
          ...
          <option value="aprobada" <?= ($filtrosListado['estado'] ?? '') === 'aprobada' ? 'selected' : '' ?>>Aprobada</option>
          <option value="rechazada" <?= ($filtrosListado['estado'] ?? '') === 'rechazada' ? 'selected' : '' ?>>Rechazada</option>
          <option value="anulada" <?= ($filtrosListado['estado'] ?? '') === 'anulada' ? 'selected' : '' ?>>Anulada</option>
        </select>
  ...
  <div class="table-responsive" style="overflow: visible;">
    <table class="table table-sm mb-0">
      <thead>
        <tr>
          <th>Número</th><th>Proveedor</th><th>Emisión</th><th>Entrega est.</th><th>Estado</th><th>N° recepción</th><th>Usuario</th><th class="text-end">Acciones</th>
        </tr>
      </thead>
      <tbody>
<?php if(empty($ordenes)): ?><tr><td colspan="8" class="text-center text-muted py-3">Sin órdenes de compra registradas.</td></tr><?php else: foreach($ordenes as $o): ?><tr><td><?= e($o['numero']) ?></td><td><?= e($o['proveedor_nombre'] ?? '-') ?></td><td><?= e($o['fecha_emision']) ?></td><td><?= e($o['fecha_entrega_estimada']) ?></td><td><?= e($o['estado_mostrado'] ?? $o['estado']) ?></td><td><?= e($o['numero_recepcion'] ?? '-') ?></td><td><?= e($o['usuario_nombre'] ?? '-') ?></td><td class="text-end"><div class="dropdown dropup"><button class="btn btn-light btn-sm dropdown-toggle" data-bs-toggle="dropdown">Acciones</button><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="<?= e(url('/app/inventario/ordenes-compra/ver/' . $o['id'])) ?>">Ver</a></li><?php if (!in_array((string) ($o['estado_mostrado'] ?? $o['estado'] ?? ''), ['aprobada', 'recepcionada'], true)): ?><li><a class="dropdown-item" href="<?= e(url('/app/inventario/ordenes-compra/editar/' . $o['id'])) ?>">Editar</a></li><?php endif; ?><li><a class="dropdown-item" href="<?= e(url('/app/inventario/ordenes-compra/pdf/' . $o['id'])) ?>">PDF</a></li><li><hr class="dropdown-divider"></li><li><form method="POST" action="<?= e(url('/app/inventario/ordenes-compra/estado/' . $o['id'])) ?>" onsubmit="return confirm('¿Aprobar esta orden de compra?')"><?= csrf_campo() ?><input type="hidden" name="estado" value="aprobada"><button type="submit" class="dropdown-item text-success">Aprobar</button></form></li><li><form method="POST" action="<?= e(url('/app/inventario/ordenes-compra/estado/' . $o['id'])) ?>" onsubmit="return confirm('¿Rechazar esta orden de compra?')"><?= csrf_campo() ?><input type="hidden" name="estado" value="rechazada"><button type="submit" class="dropdown-item text-danger">Rechazar</button></form></li></ul></div></td></tr><?php endforeach; endif; ?>
      </tbody>
    </table>
  </div>
</div>
