...
<form method="POST" class="d-grid gap-3" id="form-cotizacion-editar">
    <?= csrf_campo() ?>
    <input type="hidden" name="orden_compra_origen_id" id="orden_compra_origen_id" value="<?= e((string) ($cotizacion['orden_compra_origen_id'] ?? '')) ?>">
        ...
        <div class="card-body row g-3">
            ...
            <div class="col-md-6">
                ...
                <div class="form-check form-switch">
                    ...
                    <label class="form-check-label" for="switch_oc_aprobada">Reemplazar ítems con una orden de compra aprobada</label>
                </div>
...
(function () {
    ...
    const ordenOrigenActual = String(document.getElementById('orden_compra_origen_id')?.value || '');
    if (switchOcAprobada && selectOcAprobada && ordenOrigenActual !== '') {
        const existe = Array.from(selectOcAprobada.options).some((opt) => String(opt.value) === ordenOrigenActual);
        if (existe) {
            switchOcAprobada.checked = true;
            selectOcAprobada.classList.remove('d-none');
            selectOcAprobada.value = ordenOrigenActual;
        }
    }
    aplicarListaATodasLineas(true);
})();
